...
namespace ItemDoc.Framework.Environment
{
  /// <summary>
  ///  网站config中配置
  /// </summary>
  public static class Config
  {
    ...
    /// <summary>
    /// <appSettings><add key="设置key" value="设置value"/></appSettings> 
    /// <example>AppSettings.(key的名字)  显示对应的value</example>
    /// </summary>
    public static readonly string BeianIcp = ConfigurationManager.AppSettings["BeianICP"] ?? string.Empty;
    /// <summary>
    /// 允许上传及删除的文件类型(小写，可在*.config中AppSettings区域AllowUploadFile项自定义配置)。
    /// 如果没有配置此项值将返回string.Empty，即不允许上传及删除任何文件！
    /// </summary>
    public static readonly string AllowUploadOrDeleteFileTypes = ConfigurationManager.AppSettings["AllowUploadFile"] ?? string.Empty;
    /// <summary>
    /// DES解密字符串
    /// </summary>
    public static readonly string EncryptKey = ConfigurationManager.AppSettings["EncryptKey"] ?? "sop@cce#.￥com";
    ...
    /// <summary>
    /// 获取AppSetting里面的内容,没有值返回默认输入的值
    /// </summary>
    /// <typeparam name="T">Value类型，一般都是string</typeparam>
    /// <param name="Value">AppSetting里面的Value值</param>
    /// <param name="key">AppSetting里面的Key值</param>
    /// <returns>AppSetting里面的Value值</returns>
    public static T GetAppSettingValue<T>(T Value, string key)
    {
